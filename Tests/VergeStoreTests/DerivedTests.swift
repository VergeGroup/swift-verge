...
final class DerivedCacheTests: XCTestCase {
  ...
  func test_identify_keypath_specify_queue_any() {
    ...
    let queue = Scheduler.specific(DispatchQueue(label: "test"))
    let queue2 = Scheduler.specific(DispatchQueue(label: "test"))
  ...
  func test_identify_keypath_specify_queue_global() {
    ...
    let queue = Scheduler.specific(DispatchQueue.global())
